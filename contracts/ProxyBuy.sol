...
contract ProxyBuy is SafeMath, owned{
    
    ARCToken public token;
    address whitelisted;
    mapping (address => uint) public proxybalances;
    
    function ProxyBuy(address _tokenaddress){
        token = ARCToken(_tokenaddress);
        whitelisted = msg.sender;
    }

    function setWhitelist(address _allowed) onlyowner{
        whitelisted = _allowed;
    }
    
    // can only be executed by the whitelisted account
    function buyFor (address _target) payable{
        if (msg.sender != whitelisted) throw;
        uint previousBalance = token.balanceOf(this);
        if (!token.call.gas(100000).value(msg.value)()) throw;
        uint newBalance = safeSub(token.balanceOf(this),previousBalance);
        proxybalances[_target] = safeAdd(proxybalances[_target],newBalance);
    }
    
    // withdraw someone's tokens to his address
    // can only be executed by the whitelisted account
    function withdrawFor(address _target) {
        if (msg.sender != whitelisted) throw;
        if (!token.transfer(_target,proxybalances[_target])) throw;
        proxybalances[_target] = 0;
    }

    // withdraw your own tokens
    function withdraw() {
        if (!token.transfer(msg.sender,proxybalances[msg.sender])) throw;
        proxybalances[msg.sender] = 0;
    }
